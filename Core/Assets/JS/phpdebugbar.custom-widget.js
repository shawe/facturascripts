...
(function ($) {
    ...
    var SQLQueriesWidget = PhpDebugBar.Widgets.SQLQueriesWidget = PhpDebugBar.Widget.extend({
        ...
        onFilterClick: function (el) {
            ...
            this.$list.$el.find("li[connection=" + $(el).attr("rel") + "]")
                .toggle();
        ...
        render: function () {
            ...
            this.$list = new PhpDebugBar.Widgets.ListWidget({
                itemRenderer: function (li, stmt) {
                    ...
                    if (stmt.stmt_id !== undefined && stmt.stmt_id) {
                    ...
                    if (stmt.is_success !== undefined && !stmt.is_success) {
            ...
            this.bindAttr("data", function (data) {
                    ...
                    if (sql[stmt].keys.length > 1) {
                        ...
                        for (var j = 0; j < sql[stmt].keys.length; j++) {
                            this.$list.$el.find("." + csscls("list-item"))
                                ...
                                .addClass(csscls("sql-duplicate-" + unique));
                        }
                    }
                }
                ...
                if (data.accumulated_duration_str) {
                    this.$status.append($("<span title='Accumulated duration' />")
                        .addClass(csscls("duration"))
                        .text(data.accumulated_duration_str));
                }
                if (data.memory_usage_str) {
                    this.$status.append($("<span title='Memory usage' />")
                        .addClass(csscls("memory"))
